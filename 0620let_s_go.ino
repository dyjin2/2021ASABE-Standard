...
#define R_infraredray 47   //定义右红外传感器接口引脚 A10
...
void TrackStraight(int step){
  ...
  for(lyy=0;lyy<step;lyy++){
    ...
    //该版本红外高电平输出0，低电平输出1
    if(b[0]==0&& b[1]==0)            //两个红外都不亮，直走
      {GoStraight(20);}
    else if(b[0]==0 && b[1]==1)   //车头向右前方，应该往左边调整，车向左转
      {TrackStraightLeft(1);
      }
    else if(b[0] ==1 && b[1]==0)   //车头向左前方，应该往右边调整
...
void Servo_Setupfinal() {
  pinMode(Servo_1, OUTPUT);           //设定 Servo_1 引脚为输出模式，以控制舵机
  My_Servo.attach(Servo_1);           //将舵机引脚和我们定义的舵机 attach 起来
    for(int servoi=68;servoi>=96;servoi=servoi+2){
    My_Servo.write(servoi);
    delay(20);
  }               //写入舵机角度，让舵机在初始状态固定在 20°（根据实际情况写入角度）
  delay(500);                         //延迟一段时间，给舵机足够的转动时间到预设角度
  My_Servo.detach();                  //用完了将引脚和我们定义的舵机 detach，否则可能有 bug
}

void Servo_Setuplast() {
  pinMode(Servo_1, OUTPUT);           //设定 Servo_1 引脚为输出模式，以控制舵机
  My_Servo.attach(Servo_1);           //将舵机引脚和我们定义的舵机 attach 起来
    for(int servoi=68;servoi<=160;servoi=servoi+9){
    My_Servo.write(servoi);
    delay(100);
  }               //写入舵机角度，让舵机在初始状态固定在 20°（根据实际情况写入角度）
  delay(500);                         //延迟一段时间，给舵机足够的转动时间到预设角度
